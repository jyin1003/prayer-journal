...
export default function HistoryModal({ person, onClose }: Props) {
    ...
    return (
        ...
        >
            <div className="bg-white rounded-2xl border border-gray-200 w-full max-w-md shadow-lg flex flex-col max-h-[80vh]">
                <div className="flex items-center gap-3 p-5 pb-4 border-b border-gray-100">
                    ...
                    <h2 className="font-medium text-base flex-1 truncate">{person.name}</h2>
                                ...
                                {entry.points.map((pt, i) => {
                                    ...
                                    return (
                                        <div key={i} className="flex items-start gap-2.5">
                                            <span className={`mt-1.5 w-1.5 h-1.5 rounded-full flex-shrink-0 ${ticked ? 'bg-green-400' : 'bg-gray-300'}`} />
